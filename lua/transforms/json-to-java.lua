...
M.execute = function()
  ...
  local jobid = vim.fn.jobstart("tconv json java", {
    stdout_buffered = false,
    on_stdout = append_data,
    on_stderr = append_data,
    on_exit = function(_, code, _)
      if code == 0 then
        vim.api.nvim_buf_set_option(curr_buf, 'filetype', 'java')
      end
    end,
  })
...
vim.api.nvim_create_user_command('JsonToJava', M.execute, { nargs = 0 })
